...
export function computeNextDropDate(r, now = new Date()) {
  ...
  // dateET = current window end (what most recently became bookable).
  // display = next upcoming drop date = dateET + 1 (the next drop fires tomorrow).
  const displayDate = new Date(restaurantDate)
