...
const SITE_URL = 'https://scoopd.nyc'

async function callNotifyMonitor() {
  const secret = process.env.CRON_SECRET
  if (!secret) return
  try {
    await fetch(`${SITE_URL}/api/notify-monitor`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${secret}`, 'Content-Type': 'application/json' },
      body: '{}',
    })
  } catch (err) {
    console.error(`[ot-monitor] notify-monitor call failed (non-fatal): ${err.message}`)
  }
}
...
export async function checkOpenTable(restaurant) {
  ...
  if (flagged) callNotifyMonitor().catch(() => {})
