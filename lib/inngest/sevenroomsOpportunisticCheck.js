...
import { checkNSIOpportunistic } from '../monitors/sevenrooms-opportunistic'
...
export const sevenroomsOpportunisticCheck = inngest.createFunction(
  ...
  async ({ step }) => {
    const result = await step.run('check-nsi-restaurants', async () => {
      const supabase = createClient(
        process.env.NEXT_PUBLIC_SUPABASE_URL,
        process.env.SUPABASE_SERVICE_ROLE_KEY
      )
      return checkNSIOpportunistic(supabase)
    })
